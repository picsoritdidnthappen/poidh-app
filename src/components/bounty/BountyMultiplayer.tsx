  ...
  useEffect(() => {
    if (bountyId) {
      ...
      .then(async (data: OpenBounty) => {
        ...
        const degenOrEnsNames = await Promise.all(filteredAddresses.map(addr => getDegenOrEnsName(addr)));

        setParticipants({ addresses: filteredAddresses, amounts: filteredAmounts, degenOrEnsNames });
      })
        .catch(console.error);
    }
  }, [bountyId]);
