...
export default function ClaimCard({
  ...
  const signMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(
        'Failed to ban claim: ' + error.message
      );
    },
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        ...
        !shareDropdownRef.current.contains(
  ...
  const handleShareFarcaster = async () => {
    const bountyIssuerUsername =
      getDisplayUsername(
        bountyIssuer.data,
        'farcaster'
      );

    const claimIssuerUsername =
      getDisplayUsername(
        claimIssuer.data,
        'farcaster'
      );
