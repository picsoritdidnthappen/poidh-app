...
function ClaimThumb({
  ...
  if (!imageUrl) {
    return (
      <div className='flex-shrink-0 w-24 h-24 sm:w-28 sm:h-28 rounded-lg bg-white/10 animate-pulse' />
    );
  }
  ...
  return (
    <Link
      ...
      className='flex-shrink-0 w-24 h-24 sm:w-28 sm:h-28 rounded-lg overflow-hidden relative group ring-1 ring-white/10 hover:ring-white/40 transition-all duration-200'
    >
      <Image
        src={imageUrl}
...
export default function LatestClaimImages() {
  ...
  const latestClaims = activities.data?.pages
    .flatMap((page) => page.items)
    .filter(
      (tx: any) => tx.action === 'claim created' && tx.claim != null
    )
    .slice(0, 10) ?? [];
      ...
      >
        {activities.isLoading
          ? Array.from({ length: 10 }).map((_, i) => (
              <div
                key={i}
                className='flex-shrink-0 w-24 h-24 sm:w-28 sm:h-28 rounded-lg bg-white/10 animate-pulse'
              />
            ))
